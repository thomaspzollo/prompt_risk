...
def cvar_quantile_weight(p, beta_min):
    # TODO: what's this doing? is this penalizing large values of the quantile?
    if p >= beta_min:
...
def integrate_quantiles(X, b, beta_min=0.0, beta_max=1.0):
    ...
    widths = np.concatenate(
        [X_sorted, np.full((X_sorted.shape[0], 1), dist_max)], -1)
...
class OrderStatsBound:
    """Class for representing a CDF lower bound based on order statistics."""
    q: npt.ArrayLike
    ...
    def __post_init__(self):
        ...
        assert (0 <= np.min(self.q)) and (np.max(
            self.q) <= 1.0), "elements of q must be in [0, 1]"
        assert np.all((self.q[1:] - self.q[:-1]) >= 0.0
                      ), "elements of q must be monotone nondecreasing"
        assert 0 <= self.delta <= 1, "delta must be in [0, 1]"


def bisect_proposal(proposal: Callable, delta: float):
    # TODO: what's this doing?
    # TODO: install crosstab
    def f(c):
...
def berk_jones_trunc2(n: int, delta: float, k_min: int,
                      k_max: int) -> OrderStatsBound:
    assert k_max > k_min, "k_max must be greater than k_min. got {:f}, {:f}".format(
        k_min, k_max)
...
def berk_jones_one_sided(n: int, delta: float,
                         q_min: float) -> OrderStatsBound:

    def get_bound(k):
        return berk_jones_trunc(n, delta, k)

    k_opt = bisect_left(np.arange(n).tolist(),
                        q_min,
...
def berk_jones_two_sided(n: int, delta: float, q_min: float,
                         q_max: float) -> OrderStatsBound:

    def get_bound(k):
        return berk_jones_trunc(n, delta, k)

    k_min_opt = bisect_left(np.arange(n).tolist(),
                            q_min,
                            key=lambda k: get_bound(k).q[k])
    ...
    k_max_opt = bisect_left(np.arange(n).tolist(), q_max, key=key_fun)

    return get_bound2(k_max_opt).q


def invert_lcb(t, lcb, beta_vals):
    # TODO: what does lcb stand for?
...
def calc_gini(X, L, beta_min=0.0, beta_max=1.0):
    # TODO: trace through an execution of this function
    b = L
    dist_max = 1.0
    X_sorted = np.sort(X, axis=-1)
    b_lower = np.concatenate([np.zeros(1), b], -1)
